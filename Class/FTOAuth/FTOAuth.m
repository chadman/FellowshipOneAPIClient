			...
			failSelector: (SEL)aFailSelector
{
	...
	//userCreds = [NSString stringWithCString:encodeArray length:strlen(encodeArray)];
...
- (void) callFTAPIDidFinish: (OAServiceTicket *)ticket withData:(NSData *)data {
	...
	NSLog(@"A result was returned from the FT API :: %@", dataString);
	[dataString release];
	...
	if (ticket.didSucceed) {
		...
		if (![contentType isEqualToString:@"image/jpeg"]) {
			...
			NSLog(@"HTTP OAuthResult Return Data: %@", oauthResult.returnData);
