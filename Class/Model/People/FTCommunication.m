...
@implementation FTCommunication
...
+ (FTCommunication *)populateFromDictionary: (NSDictionary *)dict {
	...
	return [[[FTCommunication alloc] initWithDictionary:dict] autorelease];
}
	...
	if (results.isSucceed) {
		...
		if (![topLevel isEqual:[NSNull null]]) {	
			...
			for (NSDictionary *currentCommunication in communications) {
				[returnCommunications addObject:[FTCommunication populateFromDictionary:currentCommunication]];
			}
		}
	}
...
+ (FTCommunication *) getByCommunicationID: (NSInteger) communicationID {
	...
	FTCommunication *returnCommunication = [[[FTCommunication alloc] init] autorelease];
	...
	if (ftOAuthResult.isSucceed) {
		...
		if (![topLevel isEqual:[NSNull null]]) {		
			returnCommunication = [FTCommunication populateFromDictionary:topLevel];
		}
	}
...
- (id) initWithCoder: (NSCoder *)coder {
	...
	self = [[FTCommunication alloc] init];
